...
export const setErrorMessage = message => dispatch => dispatch({ type: 'CHANGE_ERROR_MESSGE', message });
...
export const startSearch = () => (dispatch, getState) => {
  geocode(getState().place)
    .then(({ status, address, location }) => {
      switch (status) {
        case 'OK': {
          dispatch({ type: 'GEOCODE_FETCH', address, location });
          return searchHotelByLocation(location);
        }
        case 'ZERO_RESULTS': {
          dispatch(setErrorMessage('結果が見つからないよ'));
          break;
        }
        default: {
